...
> 原理：利用 git 的分支概念，将博客的配置文件（hexo 分支，源文件内容）和静态页面文件（main 分支，博客页面显示主体）放在不同的分支，分别管理和提交。
...
* 将之前本地的博客目录中所有文件复制到 `zouledegit.github.io.git` 中去，并设置 `.gitignore` 文件（一般不用改，用默认的即可）。
...
后续写博客文章同之前一样，在新的目录 `zouledegit.github.io.git`  中操作即可。
...
* 网上下载心仪的主题至 `hexo主目录/themes/` 下。
* 更改主目录下 `_config.yml` 文件中的 `theme:` 为选择的主题文件夹名称
* 更具所选主题的说明文档修改相关配置
* 重新生成并部署即可
